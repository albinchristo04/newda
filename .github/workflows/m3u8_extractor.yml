  ...
  extract-m3u8:
    ...
    - name: Commit and push results (optional)
      ...
      run: |
        ...
        if [ -d "output" ]; then
          cp output/rbtv*.json data/extractions/ 2>/dev/null || echo "No JSON files to copy"
          cp output/rbtv*.txt data/extractions/ 2>/dev/null || echo "No TXT files to copy"
        fi
        ...
        # Find the URL file and add content
        URLS_FILE="output/rbtv_urls.txt"
        if [ -f "$URLS_FILE" ]; then
          echo "## Latest URLs (Sample):" >> data/README.md
          echo "\`\`\`" >> data/README.md
          head -20 "$URLS_FILE" >> data/README.md
          echo "\`\`\`" >> data/README.md
        fi
      ...
      run: |
        ...
        if [ -d "results" ] && [ "$(ls -A results)" ]; then
          ...
          if command -v jq >/dev/null 2>&1; then
            PLAYLIST_JSON="results/rbtv.json"
            ...
            EVENTS_JSON="results/rbtv_grouped_events.json"
          ...
          URLS_FILE="results/rbtv_urls.txt"
          if [ -f "$URLS_FILE" ]; then
            echo "" >> $GITHUB_STEP_SUMMARY
